...
整個過程**開發者不需要一直盯著**，等循環跑完再回來做最終確認就好。以下是實際案例——[go-training/mcp-workshop PR #4](https://github.com/go-training/mcp-workshop/pull/4)（新增 RFC 9207 MCP mix-up 攻擊示範，12 個檔案、+2,238 行），Copilot Review 與 `/copilot-review` Skill 自動迭代的過程：

| 輪次 | 留言數 | 主要修正                                                                                                                |
| ---- | ------ | ----------------------------------------------------------------------------------------------------------------------- |
| 1    | 2      | `writeCallbackHTML` 未跳脫 `msg` 就內嵌進 HTML，授權回應的 `error` 參數可造成 HTML/JS 注入；Mermaid 圖的 `state` 漏了值 |
| 2    | 1      | README 的程式碼片段用了省略號，與 `mcp-client/main.go` 實際內容不符，教學範例應貼近真實程式碼                           |
| 3    | 0      | 無新留言                                                                                                                |
| 4    | 3      | Mermaid 圖的 `iss` 寫成顯示名稱，應改用 issuer URL（兩處）；README 寫範例在 `client/`，實際目錄是 `mcp-client/`         |
| 5    | **0**  | **無新留言 — 全部通過**                                                                                                 |
