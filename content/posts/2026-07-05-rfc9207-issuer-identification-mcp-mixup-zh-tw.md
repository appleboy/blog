...
這不是紙上談兵，而且已經寫進 MCP 官方藍圖了。MCP 團隊在 [2026-07-28 spec release candidate 的 beta SDK 公告][mcp-beta] 裡明講：這批 beta 的**授權強化（SEP-2468）就包含「依 [RFC 9207][rfc9207] 做 `iss` 驗證」**，而且四個官方 SDK（Python、TypeScript、Go、C#）都實作了 release candidate 的核心協議變更。以官方的 [MCP Go SDK][go-sdk] 為例，它的 `AuthorizationResult` 結構現在包含 `Code`、`State`、`Iss` 三個欄位：
...
那個第三個參數 `issParameterSupported` 又是哪來的？就是 SDK 從你的授權伺服器 metadata 解析出來的。SDK 的 [`AuthServerMeta`][go-sdk-meta]（對應 RFC 8414 / OIDC discovery 文件）同樣新增了這個欄位：
