...
不同的服務都會有需要處理比較久的任務，這些任務是不能即時執行完成，才回應給前端，這樣使用者體驗會非常的差，所以這類型的任務存在資料庫或放在[消息對列][1]就是一種處理方式，接著會啟動另一個服務來消化這些非即時性的任務，而常見的處理方式就是在服務內啟動多個 Worker Node 來平行消化任務 (如上圖)。
