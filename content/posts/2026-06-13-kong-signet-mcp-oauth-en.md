---
title: "Stop Letting Every MCP Server Collect Its Own PAT: A Unified OAuth2 Front Door with Kong + Signet"
date: 2026-06-13T20:00:00+08:00
draft: false
slug: kong-signet-mcp-oauth-en
aliases:
  - /2026/06/kong-authgate-mcp-oauth-en/
share_img: /images/2026-06-13/kong-signet-mcp.png
categories:
  - AI
  - MCP
  - OAuth
...
![cover](/images/2026-06-13/kong-signet-mcp.svg)
...
These MCP servers already sit behind [Kong][kong] (almost every internal API gateway is a Kong). So the solution is natural: **attach one plugin to Kong so that every MCP server stops accepting hand-written PATs and instead uniformly requires an Signet-issued OAuth2 access token.**
...
| Pain point               | Each collects its own PAT       | Kong + Signet unified OAuth2 front door                                |
| ------------------------ | ------------------------------- | ---------------------------------------------------------------------- |
...
| Audit                    | Scattered, inconsistent formats | Identity centralized in Signet, access via Kong                        |
| Signing-key exposure     | token = account                 | Gateway only ever sees the public key; private key never leaves Signet |

The point isn't "OAuth2 can't leak" — it's that **what leaks is different**: an access token is short-lived, so even if an AI reads it, it expires before long; and the signing private key stays inside Signet forever, where Kong can't even touch it.
...
Look at the overall architecture first. Note one key design choice: **Kong does not run the OAuth flow.** It only does two things — _advertise where the flow is_, and _verify the token that comes back_. The actual Auth Code + PKCE is run by the MCP client itself against Signet.
...
- **No shared secret on the gateway.** With HS256, the gateway would have to hold Signet's signing secret — putting a forge-anything key on the edge. With RS256 + JWKS, Kong only ever sees the **public** key; the private key never leaves Signet.
- **Zero-touch key rotation.** Rotate keys in Signet's JWKS and Kong picks them up automatically (background refresh) — no Kong config change, no restart.
...
```bash
git clone https://github.com/appleboy/kong-mcp-oauth2.git
cd kong-mcp-oauth2
...
| `jwks_uri`         |          | Signet JWKS endpoint. Leave empty to auto-discover it from the issuer's AS metadata (RFC 8414, cached 1h).                     |
...
Only tokens with `type=access` are accepted; Signet refresh tokens are rejected with `401 invalid_token`.
...
Rows **5b** and **5c** are the security-critical ones — run them before going live. Rows 1–2 work against the stub demo as shipped; rows 3 onward need real tokens, so point `issuer` / `jwks_uri` at your real Signet first.

## Signet-side preflight

Before this works end-to-end, confirm a few things on Signet (decode a real **access token**, not just the `id_token`):

1. **JWKS resolves.** `GET <issuer>/.well-known/openid-configuration` → its `jwks_uri` returns a non-empty `keys` array.
2. **Access tokens are RS256.** Decode an actual access token; its header `alg` is `RS256` (not `HS256`) and its `kid` matches a key in the JWKS. Signet's default is often `JWT_SECRET` (HS256) — make sure you've moved **access tokens** (not only `id_token`) to asymmetric signing.
3. **Issuer matches.** The token's `iss` equals the plugin's `issuer` config, byte-for-byte (mind the trailing slash).
4. **`aud` binds to the resource.** The shipped configs enforce `aud`, so every token must be requested with [RFC 8707][rfc8707] resource binding: add `<gateway_origin + resource_path>` to the OAuth client's `allowed_resources` in Signet, then send `resource=<that URL>` on the token request.
...
This Kong + Signet solution cleans them all up in one move:

- **One OAuth2 front door covering every MCP**, with a single plugin config and a different `resource_path` per resource.
- **RS256 + JWKS local offline validation**, where the gateway only ever sees the public key, the private key stays inside Signet, and alg-confusion forgeries are blocked along the way.
...
- **Identity and scope passed to the backend uniformly via headers**, with audit centralized in Signet and Kong.

If your company's MCP servers are still each collecting their own PATs, this is a low-cost, standardized upgrade path that clearly improves your security posture. The full example and config files are in [appleboy/kong-mcp-oauth2][kong-mcp] — clone it and `docker compose up` to experience the whole handshake.
